...
const initServer = async () => {
    try {
        ...
        //The monitor of server,write server log into log file,print log in console;
        await server.register({
            plugin: Good,
            options: options.good
        })

        //Swagger API documentation server,auto create api document
        await server.register({
            plugin: HapiSwagger,
        ...
        //designated the path of css href && js src && img src && font path in html file ,this router is required *
        server.route({
            method: 'GET',
            path: '/static/{file*}',
            options: {
